/**
 * @file parent_kernel_approach.cu
 * @brief This file contains the implementation of the parent kernel approach
 */
...
__global__ void parent_kernel_approach(int * d_width,
		...
		float * d_cost_cube) {
			...
			// replace the double loop with a kernel call in a 9x9 2D block
			dim3 BLOCK_SIZE(window,window, 1);
			
			child_kernel<<<1, BLOCK_SIZE>>>(&x, &y, d_width, d_height, d_Y_ref, d_Y_cam, &cost, &x_proj, &y_proj, &cc);
			printf("x: %d, y: %d, zi: %d\n", x, y, zi);
